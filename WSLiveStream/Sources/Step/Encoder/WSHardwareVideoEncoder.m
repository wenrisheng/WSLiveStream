...
@interface WSHardwareVideoEncoder (){
...
@property (nonatomic, strong) NSData *vps; // H265才有
...
@implementation WSHardwareVideoEncoder
...
- (void)resetCompressionSession {
    ...
    OSStatus status;
    if (self.configuration.enableH264) { // H264编码
        status = VTCompressionSessionCreate(NULL, _configuration.videoSize.width, _configuration.videoSize.height, kCMVideoCodecType_H264, NULL, NULL, NULL, VideoCompressonOutputCallback, (__bridge void *)self, &compressionSession);
    } else { // H265编码
        status = VTCompressionSessionCreate(NULL, _configuration.videoSize.width, _configuration.videoSize.height, kCMVideoCodecType_HEVC, NULL, NULL, NULL, VideoCompressonOutputCallback, (__bridge void *)self, &compressionSession);
    }
    ...
    OSStatus prepareStatus = VTCompressionSessionPrepareToEncodeFrames(compressionSession);
    if(prepareStatus != noErr) {
        NSLog(@"WSHardwareVideoEncoder : VTCompressionSessionPrepareToEncodeFrames Failed, status = %d",prepareStatus);
    }
...
- (void)setDelegate:(id<WSVideoEncodingDelegate>)delegate {
...
static void VideoCompressonOutputCallback(void *VTref, void *VTFrameRef, OSStatus status, VTEncodeInfoFlags infoFlags, CMSampleBufferRef sampleBuffer){
    ...
    WSHardwareVideoEncoder *videoEncoder = (__bridge WSHardwareVideoEncoder *)VTref;
    if (status != noErr) {
        return;
    }
    ...
    if (keyframe && !videoEncoder.sps) {
        ...
        if (videoEncoder.configuration.enableH264) { // H264
            ...
            size_t parmCount;
            OSStatus vpsStatus = CMVideoFormatDescriptionGetHEVCParameterSetAtIndex(format, 0, &vps, &vpsSize, &parmCount, 0);
            if (vpsStatus == noErr) {
                OSStatus spsStatus = CMVideoFormatDescriptionGetHEVCParameterSetAtIndex(format, 1, &sps, &spsSize, &parmCount, 0);
                if (spsStatus == noErr) {
                    OSStatus ppsStatus = CMVideoFormatDescriptionGetHEVCParameterSetAtIndex(format, 2, &pps, &ppsSize, &parmCount, 0);
                    if (ppsStatus == noErr) {
        ...
        if (statusCodeRet == noErr) {
            ...
            while (bufferOffset < totalLength - AVCCHeaderLength) {
                ...
                WSVideoFrame *videoFrame = [WSVideoFrame new];
...
- (void)initForFilePath {
    NSString *path = @"";
    if (self.configuration.enableH264) {
        path = [self GetFilePathByfileName:@"IOSCamDemo.h264"];
    } else {
        path = [self GetFilePathByfileName:@"IOSCamDemo.asf"];
    }
