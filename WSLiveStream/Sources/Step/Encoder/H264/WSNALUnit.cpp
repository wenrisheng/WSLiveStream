...
bool
WSNALUnit::GetStartCode(const BYTE *& pBegin, const BYTE *& pStart, int& cRemain){
...
bool
WSNALUnit::Parse(const BYTE *pBuffer, int cSpace, int LengthSize, bool bEnd){
...
void
ScalingList(int size, WSNALUnit *pnalu){
    long lastScale = 8;
    long nextScale = 8;
...
bool
LFSeqParamSet::Parse(WSNALUnit *pnalu){
    if (pnalu->Type() != WSNALUnit::NAL_Sequence_Params) {
        return false;
    }
...
// --- slice header --------------------
bool
LFSliceHeader::Parse(WSNALUnit *pnalu){
    switch (pnalu->Type()) {
    case WSNALUnit::NAL_IDR_Slice:
    case WSNALUnit::NAL_Slice:
    case WSNALUnit::NAL_PartitionA:
...
LFSEIMessage::LFSEIMessage(WSNALUnit *pnalu){
    m_pnalu = pnalu;
