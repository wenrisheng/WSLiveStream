...
@interface WSAVEncoder ()

{
    // initial writer, used to obtain SPS/PPS from header
    WSVideoEncoder *_headerWriter;

    // main encoder/writer
    WSVideoEncoder *_writer;
...
@implementation WSAVEncoder

@synthesize bitspersecond = _bitspersecond;

+ (WSAVEncoder *)encoderForHeight:(int)height andWidth:(int)width bitrate:(int)bitrate {
    WSAVEncoder *enc = [WSAVEncoder alloc];
...
- (void)initForHeight:(int)height andWidth:(int)width bitrate:(int)bitrate {
    ...
    _headerWriter = [WSVideoEncoder encoderForPath:path Height:height andWidth:width bitrate:(int)self.bitrate];
    _times = [NSMutableArray arrayWithCapacity:10];

    // swap between 3 filenames
    _currentFile = 1;
    _writer = [WSVideoEncoder encoderForPath:[self makeFilename] Height:height andWidth:width bitrate:(int)self.bitrate];
...
- (BOOL)parseParams:(NSString *)path {
    ...
    WSMP4Atom *movie = [WSMP4Atom atomAt:0 size:(int)s.st_size type:(OSType)('file') inFile:file];
    WSMP4Atom *moov = [movie childOfType:(OSType)('moov') startAt:0];
    WSMP4Atom *trak = nil;
    if (moov != nil) {
        for (;; ) {
            trak = [moov nextChild];
            ...
            if (trak.type == (OSType)('trak')) {
                WSMP4Atom *tkhd = [trak childOfType:(OSType)('tkhd') startAt:0];
    ...
    WSMP4Atom *stsd = nil;
    if (trak != nil) {
        WSMP4Atom *media = [trak childOfType:(OSType)('mdia') startAt:0];
        if (media != nil) {
            WSMP4Atom *minf = [media childOfType:(OSType)('minf') startAt:0];
            if (minf != nil) {
                WSMP4Atom *stbl = [minf childOfType:(OSType)('stbl') startAt:0];
                if (stbl != nil) {
    ...
    if (stsd != nil) {
        WSMP4Atom *avc1 = [stsd childOfType:(OSType)('avc1') startAt:8];
        if (avc1 != nil) {
            WSMP4Atom *esd = [avc1 childOfType:(OSType)('avcC') startAt:78];
            if (esd != nil) {
    ...
    @synchronized(self){
        ...
        if (!_swapping) {
            ...
            if (st.st_size > OUTPUT_FILE_SWITCH_POINT || self.bitrateChanged) {
                ...
                _swapping = YES;
                WSVideoEncoder *oldVideo = _writer;
                ...
                _writer = [WSVideoEncoder encoderForPath:[self makeFilename] Height:_height andWidth:_width bitrate:(int)self.bitrate];
...
- (void)onNALU:(NSData *)nalu {
    ...
    if (_pendingNALU) {
        WSNALUnit nal(pNal, (int)[nalu length]);
