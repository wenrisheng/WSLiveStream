...
@implementation WSMP4Atom

@synthesize type = _type;
@synthesize length = _length;

+ (WSMP4Atom *)atomAt:(int64_t)offset size:(int)length type:(OSType)fourcc inFile:(NSFileHandle *)handle {
    WSMP4Atom *atom = [WSMP4Atom alloc];
...
- (WSMP4Atom *)nextChild {
    if (_nextChild <= (_length - 8)) {
        ...
        _nextChild += len;
        len -= cHeader;
        return [WSMP4Atom atomAt:offset+_offset size:(int)len type:fourcc inFile:_file];
    }
    return nil;
}

- (WSMP4Atom *)childOfType:(OSType)fourcc startAt:(int64_t)offset {
