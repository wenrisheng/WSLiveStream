...
@interface WSStreamingBuffer (){
    dispatch_semaphore_t _lock;
}

@property (nonatomic, strong) NSMutableArray <WSFrame *> *sortList;
@property (nonatomic, strong, readwrite) NSMutableArray <WSFrame *> *list;
...
@implementation WSStreamingBuffer
...
#pragma mark -- Custom
- (void)appendObject:(WSFrame *)frame {
    if (!frame) return;
    if (!_startTimer) {
        _startTimer = YES;
        [self tick];
    ...
    if (self.sortList.count < defaultSortBufferMaxCount) {
        ...
        /// 添加至缓冲区
        WSFrame *firstFrame = [self.sortList lfPopFirstObject];
...
- (WSFrame *)popFirstObject {
    dispatch_semaphore_wait(_lock, DISPATCH_TIME_FOREVER);
    WSFrame *firstFrame = [self.list lfPopFirstObject];
    dispatch_semaphore_signal(_lock);
    return firstFrame;
}
...
- (NSArray *)expirePFrames {
    ...
    for (NSInteger index = 0; index < self.list.count; index++) {
        WSFrame *frame = [self.list objectAtIndex:index];
        if ([frame isKindOfClass:[WSVideoFrame class]]) {
            WSVideoFrame *videoFrame = (WSVideoFrame *)frame;
...
- (NSArray *)expireIFrames {
    ...
    for (NSInteger index = 0; index < self.list.count; index++) {
        WSFrame *frame = [self.list objectAtIndex:index];
        if ([frame isKindOfClass:[WSVideoFrame class]] && ((WSVideoFrame *)frame).isKeyFrame) {
...
NSInteger frameDataCompare(id obj1, id obj2, void *context){
    WSFrame *frame1 = (WSFrame *)obj1;
    WSFrame *frame2 = (WSFrame *)obj2;
